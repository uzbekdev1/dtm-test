...
namespace OmrMarkEngine.Output
{
    ...
    public abstract class OmrOutputDataCollection : OmrOutputData
    {
        ...
        {
            Details = new List<OmrOutputData>();
        }
        ...
        public bool AlreadyAnswered(OmrOutputData data)
        {
            ...
            var existing = Details.OfType<OmrBubbleData>().FirstOrDefault(o => o.Key == bubble.Key);
            if (existing == null)
                return false;
            if (existing.BlobArea < bubble.BlobArea)
        ...
        public virtual ValidationResult Validate(OmrTemplate template)
        {
            ...
            var requiredFields =
                new List<OmrQuestionField>(
                    template.Fields.Where(o => o.IsQuestionRequired && (o.AnswerRowGroup == rowId)));
            ...
            foreach (var itm in Details)
                ...
                else if (itm is OmrBubbleData)
                {
                    ...
                    var templateData =
                        template.Fields.Find(o => (o.Id == itm.Id) && (o.AnswerRowGroup == rowId)) as OmrBubbleField;
                    var otherOptions =
                        template.Fields.FindAll(
                            o =>
                                o is OmrBubbleField && ((o as OmrBubbleField).Question == bubbleData.Key) &&
                                (o.AnswerRowGroup == rowId));

                    // Mutli not permitted
                    if ((templateData.Behavior == BubbleBehaviorType.One) &&
                        (Details.OfType<OmrBubbleData>().Count(o => otherOptions.Exists(p => p.Id == o.Id)) != 1))
                    {
                        retVal.IsValid = false;
                    ...
                    requiredFields.RemoveAll(
                        o => o is OmrBubbleField && ((o as OmrBubbleField).Question == templateData.Question));
                }
            ...
            if (rowId != null)
            {
                var uniqueQuestionCountInTpl =
                    template.Fields.Where(o => (o.AnswerRowGroup == rowId) && o.IsQuestionRequired)
                        .OfType<OmrBubbleField>()
                        .Select(o => o.Question)
                        .Distinct();
                var uniqueQuestionCountInReq =
                    requiredFields.OfType<OmrBubbleField>().Select(o => o.Question).Distinct();
                if ((uniqueQuestionCountInReq.Count() != 0) &&
                    (uniqueQuestionCountInReq.Count() != uniqueQuestionCountInTpl.Count()))
                {
                    retVal.IsValid = false;
                    var flds = string.Empty;
                    var distinctFields =
                        requiredFields.Select(o => o is OmrBubbleField ? (o as OmrBubbleField).Question : o.Id)
                            .Distinct()
                            .ToList();
