...
namespace OmrMarkEngine.Core.Processor
{
    ...
    ///     <para>
    ///         Many of the processes within formatters, connectors and the core
    ...
    ///         the core <see cref="T:System.Threading.ThreadPool" />.
    ///     </para>
    ///     <para>
    ///         This thread pool also has the ability to wait until all the
    ///         threads within the thread pool have been completed and allows for
    ///         a timeout to be specified (ie: wait 10 seconds for all tasks to finish).
    ///     </para>
    ///     <example>
    ...
    public sealed class WaitThreadPool : IDisposable
    {
        ...
        private readonly int m_concurrencyLevel = Environment.ProcessorCount;
        // True when the thread pool is being disposed
        private bool m_disposing;
        // Queue of work items
        private readonly Queue<WorkItem> m_queue;
        ...
        public WaitThreadPool(int concurrencyLevel)
        {
            ...
            m_queue = new Queue<WorkItem>(m_concurrencyLevel);
        }
        ...
        public void Dispose()
        {
            if (m_threadDoneResetEvent != null)
            {
                if (m_remainingWorkItems > 0)
                    WaitOne();

                ((IDisposable) m_threadDoneResetEvent).Dispose();
                m_threadDoneResetEvent = null;
                m_disposing = true;
                lock (m_queue)
                {
                ...
                if (m_threadPool != null)
                    for (var i = 0; i < m_threadPool.Length; i++)
                    {
                        m_threadPool[i].Join();
                        m_threadPool[i] = null;
                    }
        ...
        public void QueueUserWorkItem(WaitCallback callback, object state)
        {
            ...
            lock (m_threadDoneResetEvent)
            {
                m_remainingWorkItems++;
            }
        ...
        public bool WaitOne(int timeout, bool exitContext)
        {
            ThrowIfDisposed();
            DoneWorkItem();
            var rv = m_threadDoneResetEvent.WaitOne(timeout, exitContext);
            lock (m_threadDoneResetEvent)
            {
                if (rv)
                {
                    m_remainingWorkItems = 1;
                    m_threadDoneResetEvent.Reset();
                }
                else m_remainingWorkItems++;
            }
            return rv;
        }
        ...
        private void DoneWorkItem()
        {
            ...
            {
                --m_remainingWorkItems;
                if (m_remainingWorkItems == 0) m_threadDoneResetEvent.Set();
            }
        }
        ...
        {
            if (m_threadDoneResetEvent == null) throw new ObjectDisposedException(GetType().Name);
        }
