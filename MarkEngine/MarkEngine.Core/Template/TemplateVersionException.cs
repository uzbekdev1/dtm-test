...
namespace OmrMarkEngine.Core.Template
{
    ...
    public class TemplateVersionException : Exception
    {
        ...
        public TemplateVersionException(string templateVersion, string maxSupportedVersion)
            : base(
                string.Format(
                    "The supplied template was created a version not compatible with this version of OMR Mark Engine. The maximum version this engine supports is {0}, the template version is {1}",
                    maxSupportedVersion, templateVersion))
        {
        }
    }
}